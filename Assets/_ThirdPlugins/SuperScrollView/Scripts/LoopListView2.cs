...
namespace SuperScrollView
{
    ...
    {
        [SerializeField]
        public bool mAutoUpdate = true;
        [SerializeField]
        public bool mAutoContentSize = true;
        ...
        */
        ...
        void AdjustPanelPos()
        {
            ...
            else if (mArrangeType == ListItemArrangeType.BottomToTop)
            {
                ...
                // fix: 修复宽屏长屏幕MovePanelToItemIndex定位不准确的bug
        ...
        {
            if (!mAutoContentSize) return;
            
            float size = GetContentPanelSize();
            SetContentSize(size);
        }

        public void RemoveEmptyItem(int i = 0)
        {
            for (; i < mItemList.Count; ++i)
            {
                if (mItemList[i] == null || mItemList[i].gameObject == null)
                {
